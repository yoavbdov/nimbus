...
function selectContentClass() {
  return "bg-popover ring-1 ring-foreground/15 shadow-depth-xl";
}
...
export function FilterBuilder({
  ...
  const [op, setOp] = useState<string>(initial?.op ?? "");
  ...
  function commit(filter: PlayerFilter) {
    onSubmit(filter);
  }
  ...
  function submit() {
    ...
    if (mode === "none") {
      commit(buildFilter(field, opDef.op, "none", "", initial?.id));
      return;
    }
    if (!isValueComplete(mode, currentValue)) return;
    commit(buildFilter(field, opDef.op, mode, currentValue, initial?.id));
  }
    ...
    >
      ...
      <AnimatePresence initial={false}>
        {showValueStep && fieldDef && opDef && mode && (
          <StepShell
            key="value"
            width={mode === "multi-enum" ? "w-56" : "min-w-36 w-fit"}
          >
